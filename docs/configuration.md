...
| `inactiveColor` | string                               | theme badge  | Badge color while **off** — closed, locked or docked, whichever this entity's domain says. Ignored while `stateColor` rules match. See [Colors for on and off](behavior.md#colors-for-on-and-off). |
